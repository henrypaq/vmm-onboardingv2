-- =====================================================
-- DEFINITIVE COMPLETE DATABASE SETUP - UPDATED
-- =====================================================
-- This script creates ALL necessary tables, indexes, policies, triggers, and functions
-- for the complete VMM onboarding platform with EXACT schema matching.
...
-- =====================================================
-- 4. ONBOARDING LINKS TABLE - UPDATED VERSION
-- =====================================================
-- Generated onboarding links with platform and permission specifications
-- FORCE DROP and recreate to ensure exact schema match and refresh cache
...
CREATE TABLE onboarding_links (
  ...
  is_used boolean DEFAULT false, -- NEW: Track if link has been used
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
CREATE INDEX IF NOT EXISTS idx_onboarding_links_is_used ON onboarding_links(is_used); -- NEW: Index for is_used
...
DO $$
BEGIN
  RAISE NOTICE '🔍 Verifying onboarding_links table structure...';
  RAISE NOTICE '✅ Table should have: id, admin_id, client_id, link_name, token, platforms, requested_permissions, expires_at, status, is_used, created_at, updated_at';
END $$;
...
DO $$
BEGIN
  RAISE NOTICE '✅ DEFINITIVE database setup completed successfully!';
  ...
  RAISE NOTICE '✅ PERFECT SCHEMA MATCH: All columns match code expectations';
...
-- SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'onboarding_links' ORDER BY ordinal_position;
